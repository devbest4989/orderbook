...
var PurchaseOrdersEdit = function(){
  var initEditOrderDetailForm = function(){
    ...
    $('#purchase_order_ship_country').addClass('form-control');
    $('#purchase_order_bill_country').addClass('form-control');
  }
  ...
  var handleOrderDetail = function(){
    ...
    $('#purchase_order_supplier_id').on('change', function(e) {
      detail_url = "/supplier/detail_info/" + $(this).find('option:selected').val();
      $.ajax({
        url: detail_url,
        type: "post",
        datatype: 'json',
    ...
    $('.btn-order-detail').click(function() {      
      ...
      validateFront('#edit_order_detail_form');
      if(validate === true){
        var message = ($('#save_action').val() == 'draft') ? "Do you want to send draft email to supplier?"  : "Do you want to send confirmation email to supplier?";
        ...
        var reqUrl = "/purchase_orders/" + $('#purchase_order_id').val();
        $.ajax({
          url: reqUrl,
          type: "post",
          datatype: 'json',
    ...
    $('.btn-order-product').click(function() {
      ...
      if(validate === true){
        ...
        reqData = [];
        reqData.push({name: 'purchase_order[total_amount]', value: $('#total_cell').text().trim()});
