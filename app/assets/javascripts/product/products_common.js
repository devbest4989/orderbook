...
var SubProducts = function(){
  var initialNewForm = function(){
    ...
    $('#sub_product_purchase_tax_id').change(function(){
      calculatePurchasePrice();
    });

    $('#sub_product_purchase_price').change(function(){
      calculatePurchasePrice();
    });

    $('#sub_product_purchase_price_ex').change(function(){
      calculatePurchasePrice();
    });
    ...
    $('#sub_product_selling_tax_id').change(function(){
      calculateSellingPrice();
    });
  ...
  var handleImageUploader = function (){
    var btnCust = '<button type="button" class="btn btn-default" title="Add picture tags" ' + 
      ...
      '<i class="glyphicon glyphicon-tag"></i>' +
      '</button>'; 
    var sub_product_image_url = ($('#sub_product_image_url').val() == '' ) ? '/images/nothumb.png' : $('#sub_product_image_url').val();
    $("#sub_product_image").fileinput({
      overwriteInitial: true,
      maxFileSize: 2000,
      showClose: false,
      showCaption: false,
      ...
      elErrorContainer: '#kv-avatar-errors',
      msgErrorClass: 'alert alert-block alert-danger',
      defaultPreviewContent: '<img src="'+sub_product_image_url+'" alt="Product Picture" style="max-width: 200px;max-height: 200px;"><h6 class="text-muted">Click to select</h6>',
      layoutTemplates: {main2: '{preview}  {remove} {browse}'},
      allowedFileExtensions: ["jpg", "png", "gif"]
    });      
  }
  ...
  var switchSellingPrice = function (){
    if(!$('#sub_product_selling_price_type').is(':checked')){
      $('.selling_exclude_tax').hide();
      $('.selling_include_tax').show();
    } else {
      $('.selling_exclude_tax').show();
  ...
  var calculateSellingPrice = function (percent = true){
    var tax_string = $('#sub_product_selling_tax_id option:selected').text();
    var tax_rate = tax_string.substring(tax_string.lastIndexOf('-') + 2, tax_string.length -1);
    if(!$('#sub_product_selling_price_type').is(':checked')){
      var exclude_price = $('#sub_product_selling_price').val()/(1 + tax_rate * 0.01);
      $('#sub_product_selling_price_ex').val(exclude_price.toFixed(2));
    } else {
      var include_price = $('#sub_product_selling_price_ex').val()*(1 + tax_rate * 0.01);
      $('#sub_product_selling_price').val(include_price.toFixed(2));
    }
  ...
  var switchPurchasePrice = function (){
    if(!$('#sub_product_purchase_price_type').is(':checked')){
      $('.purchase_exclude_tax').hide();
      $('.purchase_include_tax').show();
    } else {
      $('.purchase_exclude_tax').show();
  ...
  var calculatePurchasePrice = function (){
    var tax_string = $('#sub_product_purchase_tax_id option:selected').text();
    var tax_rate = tax_string.substring(tax_string.lastIndexOf('-') + 2, tax_string.length -1);
    if(!$('#sub_product_purchase_price_type').is(':checked')){
      var exclude_price = $('#sub_product_purchase_price').val()/(1 + tax_rate * 0.01);
      $('#sub_product_purchase_price_ex').val(exclude_price.toFixed(2));
    } else {
      var include_price = $('#sub_product_purchase_price_ex').val()*(1 + tax_rate * 0.01);
      $('#sub_product_purchase_price').val(include_price.toFixed(2));
    }
  ...
  var calculatePercents = function(type){
    var purchase_price = ($('.purchase-tax-checkbox').is(':checked')) ? ($('#sub_product_purchase_price_ex').val()) : ($('#sub_product_purchase_price').val());
    if(purchase_price == 0 || purchase_price == '') return;
  ...
  return {
    initSubProductNewForm: function(){
      initialNewForm();
      handleImageUploader();
      handleTaxChecker();
    }
